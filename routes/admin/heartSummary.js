...
let report=require("../../database/model/heartSummary");
let score=require("../../database/model/scores")

router.post("/add", (req, res, next) => { //用户上传心得总结
...
router.post("/update", (req, res, next) => {
    ...
    report.update({_id: id},{$set: {img,status}}).then(data => {
        ...
        score.create({type: 5,score: 3,userId: req.session.user.idCardNumber, typeName: "上传心得总结", }).then(data => {
            console.log(data)
        })
    }).catch(err => {
        new Error(err);
...
//获取已经审核通过的汇报
router.get("/getheartSummary",(req,res,next)=>{
